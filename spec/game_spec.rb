...
describe Game do
  ...
  let(:defeated_player) { double :player, hit_points: 0 }
  subject(:game) { described_class.new(player_1, player_2) }
  subject(:finished_game) {described_class.new(defeated_player, player_2)}
  ...
  describe '#loser' do
    it 'returns a player on less than 0HP' do
      expect(finished_game.loser).to eq defeated_player
    end
  end
end
